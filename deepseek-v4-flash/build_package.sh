...
# 现场:  tar xzf dsv4-kit-09-*.tar.gz && cd dsv4-kit-09 && bash up.sh
...
build_one(){
  ...
  echo "dsv4-kit-$NN $TS (git $GIT)" > "$ROOT/VERSION"
...
EOF
  ...
  local PKG="$OUT/dsv4-kit-$NN-$TS.tar.gz"
  tar -C "$TMP" -czf "$PKG" "dsv4-kit-$NN"
