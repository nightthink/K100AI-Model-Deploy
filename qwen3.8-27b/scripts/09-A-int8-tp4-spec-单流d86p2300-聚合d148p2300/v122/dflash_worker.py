...
class DFlashWorker:
    ...
    def __init__(
        ...
        capture_offset = int(os.getenv("SGLANG_DFLASH_TARGET_CAPTURE_OFFSET", "0") or 0)
        if capture_offset:
            current_ids = list(getattr(self.model_runner, "dflash_target_layer_ids", None) or [])
            if not current_ids:
                raise RuntimeError("DFLASH target capture offset requested but no target layer ids are configured")
            shifted_ids = [int(x) + capture_offset for x in current_ids]
            target_model_for_capture = self.model_runner.model
            for module in target_model_for_capture.modules():
                if hasattr(module, "_is_layer_to_capture"):
                    delattr(module, "_is_layer_to_capture")
            target_model_for_capture.set_dflash_layers_to_capture(shifted_ids)
            self.model_runner.dflash_target_layer_ids = shifted_ids
            if self.tp_rank == 0:
                logger.warning(
                    "DFLASH target capture offset active: original=%s shifted=%s offset=%d",
                    current_ids,
                    shifted_ids,
                    capture_offset,
                )
        ...
        self._logged_selector_path = False
        self._debug_stats = os.getenv("SGLANG_DFLASH2_DEBUG_STATS", "0").strip().lower() in (
            "1",
            "true",
            "yes",
            "on",
        )
        self._debug_verify_rounds = 0
        self._debug_accepted_drafts = 0
        self._debug_offered_drafts = 0
        self._debug_diag_limit = int(os.getenv("SGLANG_DFLASH2_DEBUG_DIAG_ROUNDS", "16"))
        self._debug_last_topk0 = None
        self._debug_last_selected0 = None
        self._debug_target_hidden_dumped = False
        self._debug_target_hidden_dump_path = os.getenv(
            "SGLANG_DFLASH_DUMP_TARGET_HIDDEN", ""
        ).strip()
        self._debug_target_hidden_override_path = os.getenv(
            "SGLANG_DFLASH_OVERRIDE_TARGET_HIDDEN", ""
        ).strip()
        self._debug_target_hidden_override_payload = None
        self._debug_target_hidden_dump_ctx_len = int(
            os.getenv("SGLANG_DFLASH_DUMP_CTX_LEN", "0") or 0
        )
        ...
        # DFlash2 grouped convolution indexes positions within the resolved block.
        # Keep legacy DFlash models untouched by calling this only when supported.
        set_block_size = getattr(self.draft_model, "set_block_size", None)
        ...
        self._use_fused_kv_materialize = (is_cuda() or is_hip()) and os.getenv(
            "SGLANG_DFLASH_DISABLE_FUSED_KV", "0"
        ).strip().lower() not in ("1", "true", "yes", "on")
        self._fused_kv_helper: Optional[object] = None
        if self.tp_rank == 0 and not self._use_fused_kv_materialize:
            logger.info("DFLASH fused KV materialization disabled by runtime policy.")
    ...
    def _tp_synced_uniform(self, shape, *, device: torch.device) -> torch.Tensor:
        """Generate rank-0 random uniforms and broadcast them across the TP group."""
        if self.tp_rank == 0:
            out = torch.rand(shape, dtype=torch.float32, device=device)
        else:
            out = torch.empty(shape, dtype=torch.float32, device=device)
        tp_group = get_tp_group()
        if int(getattr(tp_group, "world_size", 1)) > 1:
            tp_group.broadcast(out, src=0)
        return out
    ...
    def _prepare_for_speculative_decoding(
        ...
        if (
            self._debug_target_hidden_dump_path
            and not self._debug_target_hidden_dumped
            and draft_input.target_hidden is not None
            and draft_input.target_hidden.numel() > 0
            and (
                self._debug_target_hidden_dump_ctx_len <= 0
                or int(draft_input.target_hidden.shape[0]) == self._debug_target_hidden_dump_ctx_len
            )
            and self.tp_rank == 0
        ):
            payload = {
                "target_hidden": draft_input.target_hidden.detach().to("cpu"),
                "ctx_lens": draft_input.ctx_lens.detach().to("cpu"),
                "draft_seq_lens": draft_input.draft_seq_lens.detach().to("cpu"),
                "batch_seq_lens": batch.seq_lens.detach().to("cpu"),
                "bonus_tokens": draft_input.bonus_tokens.detach().to("cpu"),
                "target_layer_ids": getattr(
                    self.model_runner, "dflash_target_layer_ids", None
                ),
            }
            torch.save(payload, self._debug_target_hidden_dump_path)
            logger.info(
                "DFLASH debug target hidden dumped: path=%s shape=%s dtype=%s",
                self._debug_target_hidden_dump_path,
                tuple(draft_input.target_hidden.shape),
                draft_input.target_hidden.dtype,
            )
            self._debug_target_hidden_dumped = True
        ...
        draft_prefix_lens_cpu = draft_prefix_lens.to(device="cpu", dtype=torch.int32)
        seq_lens_cpu = self._draft_seq_lens_cpu_buf[:bs]
        seq_lens_cpu.copy_(draft_prefix_lens_cpu)
        seq_lens_cpu.add_(int(self.block_size))
        ...
        try:
            if self.page_size == 1:
                ...
                block_end_cpu = draft_prefix_lens_cpu + int(self.block_size)
        ...
        stage_sync = os.getenv("SGLANG_DFLASH2_STAGE_SYNC", "0").strip().lower() in (
            "1",
            "true",
            "yes",
            "on",
        ...
        if stage_sync:
            ...
            logger.info("DFLASH2_STAGE draft_forward_done")

        selector = getattr(self.draft_model, "candidate_selector", None)
        if selector is not None:
            if not self._logged_selector_path and self.tp_rank == 0:
                logger.info(
                    "DFLASH2 Candidate Selector active. block_size=%d, selector_top_k=%d",
                    self.block_size,
                    int(getattr(selector, "top_k", -1)),
                )
                self._logged_selector_path = True
            if getattr(self.draft_model, "lm_head", None) is None:
                self.draft_model.lm_head = lm_head
            pred_hidden = draft_hidden[:, 1:, :]
            num_pred = int(pred_hidden.shape[1])
            candidate_ids, unary_logits = self.draft_model.compute_candidates(
                pred_hidden.reshape(-1, pred_hidden.shape[-1])
            )
            if stage_sync:
                torch.cuda.synchronize()
                logger.info("DFLASH2_STAGE compute_candidates_done")
            candidate_ids = candidate_ids.view(bs, num_pred, -1)
            unary_logits = unary_logits.view(bs, num_pred, -1)
            scores = selector.build_lattice(
            ...
            if stage_sync:
                torch.cuda.synchronize()
                logger.info("DFLASH2_STAGE lattice_done")
            sampling_info = batch.sampling_info
            selector_sampling = (
                sampling_info is not None and not sampling_info.is_all_greedy
            )
            if selector_sampling:
                proposal_uniforms = self._tp_synced_uniform(
                    (bs, num_pred), device=pred_hidden.device
                )
                accept_uniforms = self._tp_synced_uniform(
                    (bs, num_pred), device=pred_hidden.device
                )
                final_uniforms = self._tp_synced_uniform(
                    (bs,), device=pred_hidden.device
                )
                temperatures = (
                    sampling_info.temperatures.view(-1)
                    .to(device=pred_hidden.device, dtype=torch.float32)
                    .clamp_min_(1e-5)
                )
                greedy_mask = (sampling_info.top_ks <= 1).view(-1).to(
                    device=pred_hidden.device, dtype=torch.bool
                )
            else:
                # Frozen greedy path: preserve exact proposal math and avoid consuming RNG.
                proposal_uniforms = torch.zeros(
                ...
                greedy_mask = torch.ones(
                    (bs,), dtype=torch.bool, device=pred_hidden.device
                )

            draft_next, selector_q_rows = selector.sample_path(
                candidate_ids=candidate_ids,
                scores=scores,
                uniforms=proposal_uniforms,
                temperatures=temperatures,
                greedy_mask=greedy_mask,
            )
            if selector_sampling:
                # Rank-0 is proposal authority as well as acceptance authority.  Keep
                # ids/q/tokens identical before target verify to prevent TP state forks.
                tp_group = get_tp_group()
                if int(getattr(tp_group, "world_size", 1)) > 1:
                    tp_group.broadcast(candidate_ids, src=0)
                    tp_group.broadcast(selector_q_rows, src=0)
                    tp_group.broadcast(draft_next, src=0)
            if stage_sync:
                torch.cuda.synchronize()
                logger.info("DFLASH2_STAGE selector_walk_done")
            if self._debug_stats and self._debug_verify_rounds < self._debug_diag_limit:
                self._debug_last_topk0 = candidate_ids[:, 0, :].detach()
                self._debug_last_selected0 = draft_next[:, 0].detach()
        else:
        ...
        verify_input = DFlashVerifyInput(
            ...
            selector_candidate_ids=(candidate_ids if selector is not None and selector_sampling else None),
            selector_q_rows=(selector_q_rows if selector is not None and selector_sampling else None),
            selector_accept_uniforms=(accept_uniforms if selector is not None and selector_sampling else None),
            selector_final_uniforms=(final_uniforms if selector is not None and selector_sampling else None),
        )
    ...
    def _append_target_hidden_sequential(
        ...
        debug_dump_dir = os.getenv("SGLANG_DFLASH_LAYER_DUMP_DIR", "").strip()
        for layer_idx, layer in enumerate(self.draft_model.layers):
            ...
            if (
                debug_dump_dir
                and layer_idx == 0
                and int(ctx_hidden.shape[0]) == 30
                and int(ctx_positions[0].item()) == 0
            ):
                os.makedirs(debug_dump_dir, exist_ok=True)
                torch.save(k.detach().cpu(), os.path.join(debug_dump_dir, "layer0_ctx_k_post_rope.pt"))
                torch.save(v.detach().cpu(), os.path.join(debug_dump_dir, "layer0_ctx_v.pt"))
                torch.save(ctx_cache_loc.detach().cpu(), os.path.join(debug_dump_dir, "layer0_ctx_cache_loc.pt"))
                torch.save(ctx_positions.detach().cpu(), os.path.join(debug_dump_dir, "layer0_ctx_positions.pt"))
            ...
            if (
                ...
                torch.save(k_buf.index_select(0, ctx_cache_loc).detach().cpu(), os.path.join(debug_dump_dir, "layer0_ctx_k_readback.pt"))
                torch.save(v_buf.index_select(0, ctx_cache_loc).detach().cpu(), os.path.join(debug_dump_dir, "layer0_ctx_v_readback.pt"))
    ...
    def forward_batch_generation(
        ...
        **kwargs,
        ...
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            ...
            if self._debug_target_hidden_override_path:
                if self._debug_target_hidden_override_payload is None:
                    self._debug_target_hidden_override_payload = torch.load(
                        self._debug_target_hidden_override_path,
                        map_location="cpu",
                        weights_only=False,
                    )
                override = self._debug_target_hidden_override_payload["target_hidden"]
                expected_rows = int(extend_seq_lens.to(torch.int64).sum().item())
                if int(override.shape[0]) == expected_rows:
                    draft_input.target_hidden = override.to(
                        device=logits_output.hidden_states.device,
                        dtype=logits_output.hidden_states.dtype,
                    )
                    logger.info(
                        "DFLASH debug target-hidden override active: path=%s shape=%s",
                        self._debug_target_hidden_override_path,
                        tuple(draft_input.target_hidden.shape),
                    )
            if (
                ...
                torch.save(payload, self._debug_target_hidden_dump_path)
                logger.info(
                    "DFLASH debug target hidden dumped at prefill: path=%s shape=%s dtype=%s",
                    self._debug_target_hidden_dump_path,
                    tuple(draft_input.target_hidden.shape),
                    draft_input.target_hidden.dtype,
                )
            ...
            if stage_sync:
                torch.cuda.synchronize()
                logger.info("DFLASH2_STAGE prefill_kv_append_begin")
            self._append_target_hidden_to_draft_kv(batch, draft_input)
            if stage_sync:
                torch.cuda.synchronize()
                logger.info("DFLASH2_STAGE prefill_kv_append_done")
        ...
        profile_times = os.getenv("SGLANG_DFLASH2_PROFILE_TIMES", "0").strip().lower() in (
            "1", "true", "yes", "on"
        ) and self._debug_verify_rounds < 12
        if profile_times:
            torch.cuda.synchronize()
        ...
        if profile_times:
            torch.cuda.synchronize()
            _profile_t6 = time.perf_counter()
            logger.info(
                "DFLASH2_TIMING round=%d proposal_ms=%.3f target_verify_ms=%.3f accept_ms=%.3f mamba_commit_ms=%.3f draft_kv_append_ms=%.3f total_ms=%.3f",
                self._debug_verify_rounds + 1,
                (_profile_t1 - _profile_t0) * 1000.0,
                (_profile_t3 - _profile_t2) * 1000.0,
                (_profile_t4 - _profile_t3) * 1000.0,
                (_profile_t5 - _profile_t4) * 1000.0,
                (_profile_t6 - _profile_t5) * 1000.0,
                (_profile_t6 - _profile_t0) * 1000.0,
            )
        batch.spec_info = draft_input
        batch.forward_mode = ForwardMode.DECODE

        num_correct_drafts = sum(num_correct_drafts_per_req_cpu)
        if self.tp_rank == 0 and self._debug_stats:
            if (
                self._debug_verify_rounds < self._debug_diag_limit
                and self._debug_last_topk0 is not None
                and self._debug_last_selected0 is not None
            ):
                topk0_cpu = self._debug_last_topk0.cpu().tolist()
                selected0_cpu = self._debug_last_selected0.cpu().tolist()
                target0_cpu = new_bonus_tokens.cpu().tolist()
                for i, acc in enumerate(num_correct_drafts_per_req_cpu):
                    if int(acc) != 0:
                        continue
                    target_id = int(target0_cpu[i])
                    row = [int(x) for x in topk0_cpu[i]]
                    rank = row.index(target_id) if target_id in row else -1
                    logger.info(
                        "DFLASH2_DIAG round=%d req=%d selected0=%d target0=%d target_in_topk=%s target_rank=%d topk0=%s",
                        self._debug_verify_rounds + 1,
                        i,
                        int(selected0_cpu[i]),
                        target_id,
            ...
            offered = len(num_correct_drafts_per_req_cpu) * max(0, int(self.block_size) - 1)
            self._debug_verify_rounds += 1
            self._debug_accepted_drafts += int(num_correct_drafts)
            self._debug_offered_drafts += int(offered)
            ratio = (
                self._debug_accepted_drafts / self._debug_offered_drafts
                if self._debug_offered_drafts
                else 0.0
            )
            avg_len = (
                self._debug_accepted_drafts / self._debug_verify_rounds
                if self._debug_verify_rounds
                else 0.0
            )
            logger.info(
                "DFLASH2_STATS round=%d accepted=%s offered=%d cumulative=%d/%d ratio=%.4f avg_accept_len=%.3f",
                self._debug_verify_rounds,
                num_correct_drafts_per_req_cpu,
                offered,
                self._debug_accepted_drafts,
                self._debug_offered_drafts,
                ratio,
                avg_len,
            )
        elif not self._logged_first_verify and self.tp_rank == 0:
            logger.info(
