"""迷你链 v6：minichain5 + flash-attn 双 ABI 支持（2026-09-03）
新增：260728 构建（layout ABI）→ raw-q8 传 layout=0（DocPang v1.2.1 审计值）；
260602 legacy → 不传 layout（v5 行为）。其余与 v5 一致。原头注：
...
_LAYOUT_ABI = "2.8.3+das.opt1.dtk2604.torch290.2607280958.gebb4be"
_raw_q8 = os.getenv("MC5_RAW_Q8", "1").strip().lower() in ("1", "true", "yes", "on")
_fa2 = None
_raw_layout = None  # legacy ABI 不传 layout；260728 传 0
if _raw_q8:
    _fa_ver = _metadata.version("flash-attn")
    if _fa_ver == _LEGACY_ABI:
        import flash_attn_2_cuda as _fa2
    elif _fa_ver == _LAYOUT_ABI:
        import flash_attn_2_cuda as _fa2
        _raw_layout = 0
        print("[minichain6] flash-attn 260728（layout ABI），raw-q8 以 layout=0 启用", flush=True)
    else:
        _raw_q8 = False
        print(f"[minichain6] flash-attn={_fa_ver} 无审计 ABI，raw-q8 禁用，退回 q8split", flush=True)
...
def _q8_route(q, k, v, cu_seqlens_q, max_seqlen_q, seqused_k,
    ...
    if _raw_q8:
        ...
        if exact:
            if _seen != "raw":
                _seen = "raw"
                print("[minichain6] ACTIVE q=8 verifier -> single raw paged_attention (legacy ABI)", flush=True)
            ...
            if _raw_layout is None:
                _fa2.paged_attention(*_paged_args)
            else:
                _fa2.paged_attention(*_paged_args, _raw_layout)
            return out
        # 几何不符：不 fail-closed，安全回退 q8split

    if _seen != "split":
        _seen = "split"
        print("[minichain6] ACTIVE q=8 verifier -> 2x native q=4 (fallback)", flush=True)
...
print(f"[minichain6] repair + q8(raw={_raw_q8})/split + q16k installed", flush=True)
