...
from sglang.srt.speculative.dflash_utils import (
    ...
    compute_dflash_selector_sampling_correct_drafts_and_bonus,
...
class DFlashVerifyInput(SpecInput):
    ...
    draft_token_num: int
    # DFlash2 selector sampling metadata.  These remain None for the frozen greedy path.
    selector_candidate_ids: torch.Tensor | None = None
    selector_q_rows: torch.Tensor | None = None
    selector_accept_uniforms: torch.Tensor | None = None
    selector_final_uniforms: torch.Tensor | None = None
    ...
    def verify(
        ...
        *,
        ...
        has_selector_sampling = self.selector_candidate_ids is not None
        if has_selector_sampling:
            if sampling_info is None or sampling_info.is_all_greedy:
                raise RuntimeError(
                    "DFLASH selector sampling metadata is present for an all-greedy batch."
                )
            if (
                self.selector_q_rows is None
                or self.selector_accept_uniforms is None
                or self.selector_final_uniforms is None
            ):
                raise RuntimeError("DFLASH selector sampling metadata is incomplete.")
            correct_len, bonus = compute_dflash_selector_sampling_correct_drafts_and_bonus(
                candidates=candidates,
                next_token_logits=logits_output.next_token_logits,
                sampling_info=sampling_info,
                selector_candidate_ids=self.selector_candidate_ids,
                selector_q_rows=self.selector_q_rows,
                uniform_samples=self.selector_accept_uniforms,
                uniform_samples_for_final_sampling=self.selector_final_uniforms,
            )
            # Rank-0 is the explicit sampling authority.  This prevents any tiny
            # rank-local numerical difference from forking seq_len/cache state.
