...
_conv_copy_keepalive: list = []  # [gdn-cache] 见下方 DS 布局临时张量保活
...
def get_conv_copy_spec(
    ...
    if is_conv_state_dim_first():
        ...
        if offset > 0:
            # [gdn-cache] 末维切片非连续：materialize 成连续块再给
            # 平坦内存拷贝规格（窗口小，代价可忽略）
            src_state = state[src_block_id, :, offset:].contiguous()
            # 平坦拷贝规格只存裸指针：临时张量须保活到本步拷贝完成
            _conv_copy_keepalive.append(src_state)
            if len(_conv_copy_keepalive) > 512:
                del _conv_copy_keepalive[:256]
        else:
