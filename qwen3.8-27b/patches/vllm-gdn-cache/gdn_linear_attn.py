...
class GatedDeltaNetAttention(PluggableLayer, MambaBase):
    ...
    def _forward_core(
        ...
        if (
            ...
            and attn_metadata.num_prefills == 0
            and attn_metadata.num_decodes > 0
            and getattr(attn_metadata, "all_mode", None) is None
        ):
        ...
        # mamba_cache_mode == "all" 前缀缓存支持（gdn_prefix_cache.py）
        all_mode = getattr(attn_metadata, "all_mode", None)
        if all_mode is not None:
            from vllm.model_executor.layers.mamba.gdn_prefix_cache import (
                conv_boundary_snapshots, decode_cross_block_copy, gather_slots,
                prefill_chunked_with_checkpoints)
        ...
        if attn_metadata.num_prefills > 0:
            ...
            mixed_qkv_preconv = mixed_qkv_non_spec  # all 模式边界快照取材（conv 前）
            if all_mode is not None:
                # all 模式：conv 内核读写"最后调度块"的槽；跨块时先把旧槽搬过去
                _conv_src = gather_slots(
                    non_spec_state_indices_tensor,
                    all_mode.block_idx_last_computed).to(torch.int64)
                _conv_dst = gather_slots(
                    non_spec_state_indices_tensor,
                    all_mode.block_idx_last_scheduled).to(torch.int64)
                if has_initial_state is not None:
                    _moved = (_conv_src != _conv_dst) & has_initial_state
                    if bool(_moved.any()):
                        conv_state[_conv_dst[_moved]] = conv_state[
                            _conv_src[_moved]]
                conv_cache_indices = _conv_dst.to(torch.int32)
            else:
            ...
            assert mixed_qkv_non_spec is not None
            if all_mode is not None:
                # all 模式 decode：跨块边界先搬槽（conv+ssm），随后原地更新新槽
                decode_state_indices = decode_cross_block_copy(
                    kv_caches=(conv_state, ssm_state),
                    state_indices_2d=non_spec_state_indices_tensor,
                    blk_last_computed_d=all_mode.block_idx_last_computed,
                    blk_last_scheduled_d=all_mode.block_idx_last_scheduled,
                ).to(torch.int32)
            else:
                decode_state_indices = non_spec_state_indices_tensor[  # type: ignore[index]
        ...
        if attn_metadata.num_prefills > 0:
            ...
            if all_mode is not None:
                # all 模式：分段链式调用，块边界状态写档
                assert has_initial_state is not None
                init_slot = gather_slots(
                    non_spec_state_indices_tensor,
                    all_mode.block_idx_last_computed).to(torch.int64)
                core_attn_out_non_spec = torch.empty_like(value_non_spec)
                prefill_chunked_with_checkpoints(
                    chunk_gated_delta_rule=self.chunk_gated_delta_rule,
                    q=query_non_spec,
                    k=key_non_spec,
                    v=value_non_spec,
                    g=g_non_spec,
                    ...
                    ssm_state=ssm_state,
                    init_slot=init_slot,
                    has_initial=has_initial_state,
                    meta=all_mode,
                    core_out=core_attn_out_non_spec,
                )
                last_recurrent_state = None
                # conv 块边界快照 = 边界前 width-1 个原始输入
                conv_boundary_snapshots(
                    conv_state,
                    mixed_qkv_preconv,
                    all_mode,
                    conv_state.shape[-1],  # 权威宽度=conv 缓存末维(width-1)
                )
            else:
...
# PATCHED_vllm_model_executor_layers_mamba_gdn_linear_attn_patch_0

# PATCHED_vllm_model_executor_layers_mamba_gdn_linear_attn_patch_1

# PATCHED_vllm_model_executor_layers_mamba_gdn_linear_attn_patch_2

# PATCHED_vllm_model_executor_layers_mamba_gdn_linear_attn_patch_3

# PATCHED_vllm_model_executor_layers_mamba_gdn_linear_attn_patch_4
