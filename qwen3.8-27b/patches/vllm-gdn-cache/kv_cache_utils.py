...
def resolve_kv_cache_block_sizes(
    ...
    if any(
        ...
        # [gdn-cache] mixed mamba/attn block sizes: gcd keeps a usable
        # hash granularity (every group block_size stays divisible)
...
def unify_kv_cache_spec_page_size(
    ...
    # [gdn-cache] 若不整除的只有 MambaSpec 且其页小于 target，可 padding 到
    # target（page_size_padded 语义，0.26 同款），无需 LCM 放大
    if use_lcm_fallback:
        _nondiv = [l for l in kv_cache_spec.values()
                   if target_page_size % l.page_size_bytes != 0]
        if _nondiv and all(
                isinstance(l, MambaSpec)
                and l.page_size_bytes < target_page_size
                for l in _nondiv):
            use_lcm_fallback = False
    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        ...
        if target_page_size % layer_page_size != 0:
            if (isinstance(layer_spec, MambaSpec)
                    and layer_page_size < target_page_size):
                # [gdn-cache] 保块大小，页 padding 到 target
                new_kv_cache_spec[layer_name] = _rebuild_spec_with_target_page(
                    layer_spec, target_page_size=target_page_size, ratio=1)
                continue
...
# PATCHED_vllm_v1_core_kv_cache_utils_patch_0
