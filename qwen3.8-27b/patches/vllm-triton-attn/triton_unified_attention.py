...
def unified_attention(
    ...
    # ---- 自研补丁：prefill 下放大 BLOCK_M ----------------------------------
    # 原启发式面向 decode（每序列 1 个 query token）。本模型 GQA 比 24/4=6，
    # 6<=16 使 BLOCK_M 固定 16 → BLOCK_Q = 16//6 = 2：每个内核程序只处理
    # 2 个 query token，却各自把整段 KV 扫一遍。长 prefill 时 KV 被重读上千次，
    # 实测全注意力只跑出 14.4 TFLOPS（稠密 GEMM 的 7.3%）。
    #
    # 放大 BLOCK_M 是安全的：query_mask_0/1 会挡住越界行，超出 BLOCK_Q 的行
    # 只是与下一个块重复计算并幂等重写同一结果（原代码 BLOCK_M=16 本就有
    # 4/16 这样的浪费行，机制相同）。
    # 上限受累加器寄存器约束：acc 是 BLOCK_M × HEAD_SIZE_PADDED × 4B。
    # 默认 auto：按寄存器预算反推（acc = BLOCK_M × HEAD_SIZE_PADDED × 4B）。
    # head_size=256 → 64，实测最优；128 因寄存器溢出反而慢 46%（14.48 vs 9.90s
    # @24.9K）且改变归约顺序触发浮点平局。仅 head_size=256 经过实测验证。
    # 设 VLLM_TRITON_ATTN_PREFILL_BLOCK_M=0 可关闭本补丁退回上游行为。
    import os as _os
    _bm = _os.environ.get("VLLM_TRITON_ATTN_PREFILL_BLOCK_M", "auto")
    if _bm == "auto":
        _target = max(16, 16384 // triton.next_power_of_2(head_size))
    else:
        try:
            _target = int(_bm)
        except ValueError:
            _target = 0
    if _target > 0 and num_seqs > 0 and (q.shape[0] // num_seqs) >= 32:
        BLOCK_M = max(BLOCK_M, _target)
    # ------------------------------------------------------------------------
